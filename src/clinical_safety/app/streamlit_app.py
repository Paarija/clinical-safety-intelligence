...
MOCK_DATA_WARNING = (
    ...
    "Displayed signals, trial comparisons, and source manifest counts are synthetic placeholders."
)
...
def sidebar_nav() -> str:
    ...
    st.sidebar.caption("Safety signal workflow | GLP-1 agonists | V1")
    st.sidebar.divider()
    ...
    st.sidebar.divider()
    st.sidebar.caption(
        "Safety-signal triage; not for clinical or regulatory use."
    )
    return selection
...
def page_overview() -> None:
    ...
    **GLP-1 receptor agonists** (semaglutide, liraglutide, dulaglutide, tirzepatide, exenatide).
    ...
    grade_rows = [
        ...
        {"Grade": "D", "Meaning": "Insufficient — below thresholds, poor mapping", "Action": "Low-confidence watchlist"},
    ]
...
def _render_builtin_case_study_example() -> None:
    ...
    st.caption(
        "This built-in walkthrough is not a generated case study. It shows the expected "
