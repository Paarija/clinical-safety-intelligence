...
CRITICAL CONSTRAINTS (enforced by Guardrails, also stated in prompt):
  ...
  - No clinical recommendations (prescribing, discontinuation, treatment)
  - No diagnosis claims
...
1. Avoid claiming the drug causes the adverse event. Use hedged language: \
"reports suggest an association", "is associated with", "an elevated reporting rate was observed".
2. Avoid clinical recommendations (prescribing, discontinuation, dose adjustment, diagnosis, treatment).
3. Avoid phrases such as "proves", "definitively causes", \
...
6. Your output is for analyst review only and is not a medical or regulatory decision.
